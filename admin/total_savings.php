        ...
        <div class="content flex-grow-1 animate__animated animate__fadeIn">
            ...
            <div class="table-section">
                <h5 class="mb-3 d-flex justify-content-between align-items-center">
                    Total Savings
                    <button class="btn btn-sm btn-primary add-fund-btn">
                        ...
                        <?php foreach ($funds as $fund): ?>
                            <tr>
                                ...
                                <td class="action-buttons">
                                    <i class="fas fa-edit text-warning edit-btn" title="Edit" data-id="<?php echo $fund['id']; ?>" data-name="<?php echo htmlspecialchars($fund['name']); ?>" data-totalabc="<?php echo $fund['totalabc']; ?>" data-amount="<?php echo $fund['amount']; ?>" data-savings="<?php echo $fund['savings']; ?>"></i>
                                    <i class="fas fa-trash text-danger" title="Delete" data-id="<?php echo $fund['id']; ?>"></i>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    // Add Fund Button Event
                    document.querySelector('.add-fund-btn').addEventListener('click', function() {
                        Swal.fire({
                            title: 'Add Fund',
                            html: `
                            <input type="text" id="fund-name" class="swal2-input" placeholder="Fund Sources">
                            <input type="number" step="0.01" id="fund-totalabc" class="swal2-input" placeholder="Total ABC" oninput="calculateSavings()">
                            <input type="number" step="0.01" id="fund-amount" class="swal2-input" placeholder="Amount" oninput="calculateSavings()">
                            <input type="number" step="0.01" id="fund-savings" class="swal2-input" placeholder="Savings" readonly>
                        `,
                            showCancelButton: true,
                            confirmButtonText: 'Add',
                            preConfirm: () => {
                                const name = document.getElementById('fund-name').value.trim();
                                const totalabc = document.getElementById('fund-totalabc').value.trim();
                                const amount = document.getElementById('fund-amount').value.trim();
                                const savings = document.getElementById('fund-savings').value.trim();

                                // Validate inputs
                                if (!name || !totalabc || !amount || isNaN(totalabc) || isNaN(amount) || parseFloat(totalabc) <= 0 || parseFloat(amount) <= 0 || parseFloat(savings) < 0) {
                                    Swal.showValidationMessage('Please provide valid fund name, total ABC, amount, and savings!');
                                    return false;
                                }
                            ...
                            if (result.isConfirmed) {
                                ...
                                // Send AJAX request to add the fund
                                fetch('src/process/add_fund.php', {
                                        method: 'POST',
                                        headers: {
                                    ...
                                    .then(data => {
                                        if (data.success) {
                                            Swal.fire('Success!', 'Fund has been added.', 'success').then(() => {
                                                location.reload();
                                            });
                                        } else {
                                            Swal.fire('Error!', data.message || 'Failed to add fund.', 'error');
                                        }
                                    })
                                    .catch(() => {
                    ...
                    function calculateSavings() {
                        const totalabc = parseFloat(document.getElementById('edit-fund-totalabc').value) || 0;
                        const amount = parseFloat(document.getElementById('edit-fund-amount').value) || 0;
                        const savings = totalabc - amount;
                        document.getElementById('edit-fund-savings').value = savings.toFixed(2); // Update savings field
                    }
